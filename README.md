...
- **반응형 카드 레이아웃 개선 (Flexbox, 카드 크기 고정 방식)**: 멤버/노래/음반/굿즈/뉴스 카드 그리드(`.member-grid`, `.vgrid`, `.agrid`, `.ggrid`, `.news-grid`)를 **Flexbox(`flex-wrap` + 카드 `flex: 0 1 Npx`)** 로 구성. 카드는 `flex-grow: 0`으로 크기가 절대 늘어나지 않고 고정되며, `justify-content`는 기본값(왼쪽 정렬)을 사용해 카드가 왼쪽부터 순서대로 채워짐. 화면 폭이 좁아지면 한 줄에 들어가는 카드 개수(열 수)만 자연스럽게 줄어들고, 마지막 줄에 카드가 몇 개 남든(1개든 2개든) 억지로 늘리거나 가운데로 몰지 않고 원래 크기 그대로 왼쪽에 배치 — 남는 공간은 그대로 비워둠. (참고: 이전에는 Grid `auto-fit`이나 Flexbox `flex-grow:1`+`justify-content:center`로 마지막 줄 빈 공간을 채우려 시도했으나, 카드가 부자연스럽게 커지거나 중앙 정렬되는 문제가 있어 "카드 크기 고정 + 자동 열 개수 조정"이라는 표준적인 카드 그리드 흐름으로 최종 확정함). 브레이크포인트(1024px/600px/380px)마다 `flex-basis`만 줄여 화면 크기별 열 개수가 자연스럽게 결정되도록 함. 다양한 뷰포트(1440~375px)에서 가로 스크롤 없음과 왼쪽 정렬 배치를 Playwright로 검증
...
- **최종 업데이트**: 2026-07-06 (반응형 카드 레이아웃 재수정 — 카드 크기 고정 + 왼쪽 정렬 방식)
