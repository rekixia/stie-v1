...
- **반응형 카드 레이아웃 개선**: 멤버/노래/음반/굿즈/뉴스 카드 그리드(`.member-grid`, `.vgrid`, `.agrid`, `.ggrid`, `.news-grid`)를 `grid-template-columns: repeat(auto-fill, ...)`에서 `repeat(auto-fit, minmax(min(Npx, 100%), 1fr))`로 전환. 카드 개수가 홀수/소수라 마지막 줄이 다 채워지지 않아도 빈 트랙이 남지 않고 기존 카드가 늘어나 화면 너비를 꽉 채우도록 개선. 간격(gap)도 `clamp()`로 화면 폭에 따라 부드럽게 변하도록 조정하고, 모바일 브레이크포인트(1024px/600px/380px)의 고정 열 개수(2열 강제, 1열 강제) 규칙을 모두 최소 너비 기반의 유동적인 `auto-fit` 규칙으로 교체하여 화면 크기 변화에 자연스럽게 반응하도록 함
